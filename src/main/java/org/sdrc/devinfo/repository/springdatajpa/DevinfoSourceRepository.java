...
public interface DevinfoSourceRepository extends SourceRepository , JpaRepository<UtIndicatorEn , Long>{
	...
	@Override
	@Query("SELECT utIc FROM UtIndicatorClassificationsEn utIc WHERE utIc.IC_NId " +
			"IN (SELECT distinct utdata.source_NId FROM UtData utdata WHERE utdata.IUSNId = :iusNid AND utdata.area_NId IN (SELECT area.area_NId FROM UtAreaEn area where area.area_Level= :level) )" )
				
	public List<UtIndicatorClassificationsEn> findByIUSandLevel_Nid(@Param("iusNid") Integer iusNid,@Param("level") Integer level) throws DataAccessException;
